...
async def get_session(
    ...
    settings = get_settings()
    messages, has_more = _list_recent_messages(
        repository,
        session_id,
        message_limit or settings.session_initial_message_limit,
        before_message_id=before_message_id,
    )
    payload = to_session_detail(session, messages).model_dump(mode="json")
    payload["has_more"] = has_more
    payload["has_more_messages"] = has_more
...
async def get_session_queue(
    ...
    payload = SessionQueueRead(
        session=to_session_read(session),
        active_generation=(
            _build_generation_reads(
                repository, session.id, [active_generation], step_limit=step_limit
            )[0]
            if active_generation is not None
            else None
        ),
        queued_generations=_build_generation_reads(
            repository, session.id, queued_generations, step_limit=step_limit
        ),
        active_generation_id=active_generation.id if active_generation is not None else None,
...
async def get_session_replay(
    ...
    response_payload["has_more"] = message_has_more or _generation_reads_have_more_steps(
        generations
    )
    response_payload["has_more_messages"] = message_has_more
    response_payload["has_more_steps"] = _generation_reads_have_more_steps(generations)
