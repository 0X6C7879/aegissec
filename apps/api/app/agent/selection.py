...
from app.agent.tool_concurrency import normalize_tool_concurrency
...
class WorkflowRunnableSelector:
    ...
    def select(self, *, tasks: list[TaskNode], state: dict[str, object]) -> RunnableSelection:
        ...
        parallel_read_group = [
            task for task in selected_tasks if task.scheduler_group == "parallel_read_group"
        ]
        serialized_write_group = [
            task for task in selected_tasks if task.scheduler_group == "serialized_write_group"
        ]
    ...
    def _build_selected_task(self, task: TaskNode) -> SelectedTask:
        ...
        semantics = normalize_tool_concurrency(task=task, tool_spec=tool_spec)
        return SelectedTask(
            task_id=task.id,
            task_name=task.name,
            ...
            writes_state=semantics.writes_state,
            is_concurrency_safe=semantics.is_concurrency_safe,
            is_read_only=semantics.is_read_only,
            is_destructive=semantics.is_destructive,
            scheduler_group=semantics.scheduler_group,
            access_mode=semantics.access_mode,
            side_effect_level=semantics.side_effect_level,
            resource_keys=semantics.resource_keys,
        )
