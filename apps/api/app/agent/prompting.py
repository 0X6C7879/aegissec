...
SAFETY_SCOPE_PROMPT = (
    ...
    "names always include execute_kali_command, list_available_skills, execute_skill, and "
    "read_skill_content, and may also include MCP tool aliases listed in the capability "
    "context using the format mcp__{server}__{tool}. Use execute_skill when you want the "
    "server-side facade to resolve a specific skill without bypassing runtime approval, and "
    "use execute_kali_command only when "
