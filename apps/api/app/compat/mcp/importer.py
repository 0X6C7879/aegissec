...
def import_mcp_servers(targets: list[MCPImportTarget]) -> list[ImportedMCPServer]:
    ...
    for target in targets:
        ...
        if target.source == CompatibilitySource.OPENCODE:
            imported.extend(_parse_opencode_servers(target, payload))
            continue

        imported.extend(_parse_claude_servers(target, payload))

    imported.sort(key=lambda server: (server.source, server.scope, server.name, server.config_path))
    return imported


def _parse_claude_servers(
...
def _parse_opencode_servers(
    ...
    servers_payload = payload.get("mcp")
    if not isinstance(servers_payload, dict):
        return []
    ...
    for name, raw_config in servers_payload.items():
        if not isinstance(name, str) or not isinstance(raw_config, dict):
            continue
        opencode_type = raw_config.get("type")
        imported.append(
            _build_server(
                name=name,
                source=target.source,
                ...
                config_path=target.file_path,
                raw_config=raw_config,
                opencode_type=opencode_type if isinstance(opencode_type, str) else None,
            )
        )
    return imported
...
def _build_server(
    *,
    ...
    env = _normalize_string_map(raw_config.get("env"))
    raw_url = raw_config.get("url")
    url = raw_url if isinstance(raw_url, str) else None
    headers = _normalize_string_map(raw_config.get("headers"))
    timeout_ms = _normalize_timeout_ms(raw_config.get("timeout_ms"))
...
def _infer_target_metadata(path: Path) -> tuple[CompatibilitySource, CompatibilityScope]:
    ...
    if "/.config/opencode/" in path_string or "/.opencode/" in path_string:
        return CompatibilitySource.OPENCODE, CompatibilityScope.USER
    return CompatibilitySource.CLAUDE, CompatibilityScope.PROJECT
...
def _infer_transport(raw_config: dict[str, object], opencode_type: str | None) -> MCPTransport:
    ...
    if isinstance(raw_config.get("url"), str):
        return MCPTransport.HTTP
    return MCPTransport.STDIO
...
def _normalize_command(
    ...
    if opencode_type == "local" and isinstance(command_payload, list):
